...
class Dataset_MSMT:
    ...
    def load(self, verbose=True):
        ...
        if verbose:
            ...
            print("  ---------------------------")
            print(f"  train    | {self.num_train_pids:5d} | {len(self.train):8d}")
            print(f"  query    | {len(query_pids):5d} | {len(self.query):8d}")
            print(f"  gallery  | {len(gallery_pids):5d} | {len(self.gallery):8d}")
...
class MSMT17(Dataset_MSMT):
    ...
    def download(self):
        ...
        if osp.isdir(fpath):
            ...
            raise RuntimeError(f"Please download the dataset manually to {fpath}")
