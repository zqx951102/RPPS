...
def hm(inputs, indexes, features, momentum=0.5):
    return HM.apply(inputs, indexes, features.to(inputs.device), torch.Tensor([momentum]).to(inputs.device))
...
class HybridMemory(nn.Module):
    ...
    def forward(self, inputs, indexes, is_source=True):
        ...
        inputs = hm(inputs, indexes, self.features, self.momentum).float().to(device)
        ...
        sim = torch.zeros(labels.max() + 1, B).float().to(device)
