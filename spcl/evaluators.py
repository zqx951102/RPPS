...
def extract_dy_features(
    cfg, model, data_loader, device, is_source, memory_proposal_boxes=None, memory_target_features=None, momentum=0.2
):
    model.eval()
    sour_fea_dict = collections.defaultdict(list)
    tgt_fea = OrderedDict()
                ...
                for j in range(len(embeddings)):
                    if memory_proposal_boxes is None:
                        ...
                        if max(ious[-1]) > 0.7:
                            ...
                            try:
                                memory_proposal_boxes[img_name][ious[-1]] = (
                                    momentum * memory_proposal_boxes[img_name][ious[-1]] + (1.0 - momentum) * boxes[j]
                                )
                ...
                if memory_proposal_boxes is not None:
                    ...
                    for idx in range(len(memory_proposal_boxes[img_name])):
                        if idx in ious:
                            ...
                            tgt_fea[img_name + "_" + str(instance_id)] = memory_target_features[
                                img_store_idx + idx
                            ].unsqueeze(0)
                            instance_id += 1
        ...
        for i, (imgs, fnames, pids, _, _) in enumerate(data_loader):
            ...
            if (i + 1) % print_freq == 0:
                print(
                    ...
                    "Time {:.3f} ({:.3f})\t"
                    "Data {:.3f} ({:.3f})\t".format(
                        i + 1, len(data_loader), batch_time.val, batch_time.avg, data_time.val, data_time.avg
                    )
                )
...
def evaluate_all(
    ...
    if query is not None and gallery is not None:
        ...
        assert query_ids is not None and gallery_ids is not None and query_cams is not None and gallery_cams is not None

    # Compute mean AP
    mAP = mean_ap(distmat, query_ids, gallery_ids, query_cams, gallery_cams)
    print(f"Mean AP: {mAP:4.1%}")
    ...
    cmc_scores = {
        name: cmc(distmat, query_ids, gallery_ids, query_cams, gallery_cams, **params)
...
class Evaluator:
    ...
    def evaluate(self, data_loader, query, gallery, cmc_flag=False, rerank=False):
        ...
        results = evaluate_all(
            query_features, gallery_features, distmat, query=query, gallery=gallery, cmc_flag=cmc_flag
        )
